...
void Camera::RotateXYAbsolute ( float angle_x, float angle_y ) {
    ...
    lookAtVector = { sinf ( angleY ) * cosf ( angleX ), 
                     sinf ( angleX ),
...
void Camera::MoveUp ( float step ) {
    ...
    DirectX::XMFLOAT3A y_dir = { 0.0f, 1.0f, 0.0f };

    DirectX::XMFLOAT3A step_ = { step, step, step };
    auto step_vector = DirectX::XMLoadFloat3A ( &step_ );
    auto direction = DirectX::XMLoadFloat3A ( &y_dir );
    direction = DirectX::XMVectorMultiply ( direction, step_vector );
...
void Camera::UpdateViewMatrix () {
    ...
    DirectX::XMStoreFloat4x4 (this->viewMatrixPtr, view_matrix);
}

const DxBuffer * Camera::GetViewMatrixBuffer () const {
